...
def ingresar_datos():
    ...
    while not validar_fecha(fecha_nacimiento):
        print("Error: Fecha inválida. Debe ser en el formato YYYY-MM-DD.")
        fecha_nacimiento = input("Fecha de nacimiento (YYYY-MM-DD): ")
...
def eliminar_datos():
    documento = input("Ingrese el número de documento de la persona a eliminar: ")
    cursor.execute("""
...
def modificar_datos():
    documento = input("Ingrese el número de documento de la persona a modificar: ")
...
def listar_datos_ordenados():
    ...
    4. Orden por Edad
    """)
    opcion = input("Seleccione un criterio de ordenamiento: ")
    if opcion == "1":
